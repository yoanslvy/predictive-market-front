...
import { focusInput } from '../_utils/focusInput'
import { focusRing } from '../_utils/focusRing'
import { hasErrorInput } from '../_utils/hasErrorInput'
...
const TimeSegment = ({ segment, state }: TimeSegmentProps) => {
  ...
  return (
    <div
      ...
      className={cx(
        // base
        'relative block w-full appearance-none rounded-lg border px-2.5 py-1.5 text-left uppercase tabular-nums shadow-xs outline-hidden transition sm:text-sm',
        // border color
        'border-[#30333C]',
        // text color
        'text-gray-50',
        ...
        focusInput,
        // invalid (optional)
        'group-aria-invalid/time-input:border-red-500 group-aria-invalid/time-input:ring-2  invalid:border-red-500 invalid:ring-2 invalid:ring-red-200 group-aria-invalid/time-input:ring-red-400/20',
        {
          '!w-fit !border-none !bg-transparent !px-0 !text-gray-400 !shadow-none':
            segment.type === 'literal',
          ' border-gray-700 bg-[#30333C] text-gray-500': state.isDisabled && segment.text !== ':',
        }
      )}>
...
const TimeInput = React.forwardRef<HTMLDivElement, TimeInputProps>(
  ({ hourCycle, ...props }: TimeInputProps, ref) => {
    ...
    return (
      <div {...fieldProps} ref={innerRef} className="inline-flex w-full group/time-input gap-x-2">
...
const triggerStyles = tv({
  base: [
    // base
    'peer flex w-full cursor-pointer appearance-none items-center gap-x-2 truncate rounded-lg border px-3 py-2 shadow-xs outline-hidden transition-all sm:text-sm',
    // background color
    'bg-[#202228]',
    // border color
    'border-[#30333C]',
    ...
    'disabled:border-[#30333C] disabled:bg-[#30333C] disabled:text-gray-500',
    // focus
    focusInput,
    // invalid (optional)
...
const Trigger = React.forwardRef<HTMLButtonElement, TriggerProps>(
  (
    { className, children, placeholder, hasError, tooltip, ...props }: TriggerProps,
    forwardedRef
  ) => {
    return (
      <PopoverPrimitives.Trigger asChild>
        <button
          ...
          {...props}>
          <RiCalendar2Fill className="text-gray-600 size-5 shrink-0" />
          <span className="flex-1 text-[16px] overflow-hidden text-left  text-ellipsis whitespace-nowrap text-gray-50">
            {children ? (
              children
            ) : placeholder ? (
        ...
        return (
          <li key={`preset-${preset.label}`} className="sm:w-full sm:py-px">
            <button
              ...
              className={cx(
                // base
                'relative w-full overflow-hidden rounded-lg border px-2.5 py-1.5 text-left text-base text-ellipsis whitespace-nowrap shadow-xs outline-hidden transition-all sm:border-none sm:py-2 sm:text-sm sm:shadow-none',
                // text color
                'text-gray-300',
                // border color
                'border-[#30333C]',
                ...
                'hover:bg-gray-900',
                {
                  'bg-gray-900': matchesCurrent(preset),
                }
              )}
        ...
        <div className="flex">
          <div className="flex flex-col sm:flex-row sm:items-start">
            {presets && presets.length > 0 && (
              <div
                className={cx(
                  ...
                  'border-b  sm:border-r sm:border-b-0 border-[#30333C]',
                  'overflow-auto'
                )}>
            ...
            <div>
              ...
              />
              {showTimePicker && (
                <div className="p-3 border-t border-[#30333C]">
                  <TimeInput
                    aria-label="Time"
              ...
              <div className="flex items-center p-3 border-t  gap-x-2 border-[#30333C]">
                <button
                  className="h-8 px-4 font-medium text-white transition-colors border border-[#30333C] rounded-lg "
                  onClick={onReset}>
                  Reset
                </button>
                <button
                ...
                <button
                  className="px-4 h-8 rounded-lg bg-[#01EB5A29] hover:bg-[#01EB5A40] text-[#01EB5A] font-medium transition-colors "
                  onClick={onApply}>
                  Apply
                </button>
              </div>
...
const RangeDatePicker = ({
  ...
  const displayRange = React.useMemo(() => {
    ...
    const r = `${range.from ? formatDate(range.from, locale, showTimePicker) : ''} - ${
          ...
          <div className="flex flex-col overflow-x-auto sm:flex-row sm:items-start">
            ...
            <div className="overflow-x-auto">
              <CalendarPrimitive
                ...
                initialFocus
                classNames={{
                  months: 'flex flex-row divide-x divide-[#30333C] overflow-x-auto',
                }}
                {...props}
              />
              {showTimePicker && (
                <div className="flex items-center p-3 border-t  justify-evenly gap-x-3 border-[#30333C]">
                  <div className="flex items-center flex-1 gap-x-2">
                    <span className="text-white">{translations?.start ?? 'Start'}:</span>
                    <TimeInput
                      value={startTime}
                  ...
                  <RiSubtractFill className="text-gray-400 size-4 shrink-0" />
                  <div className="flex items-center flex-1 gap-x-2">
                    <span className="text-white">{translations?.end ?? 'End'}:</span>
                    <TimeInput
                      value={endTime}
              ...
              <div className="p-3 border-t border-[#30333C] sm:flex sm:items-center sm:justify-between">
                <p className=" tabular-nums text-gray-50">
                  <span className="font-medium">{displayRange}</span>
                </p>
                <div className="flex items-center mt-2 ml-2 gap-x-2 sm:mt-0">
                  <button
                    className="h-8 px-4 font-medium text-white transition-colors border border-[#30333C] rounded-lg "
                    onClick={onReset}>
                    Reset
