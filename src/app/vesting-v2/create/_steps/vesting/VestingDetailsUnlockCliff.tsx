...
export function VestingDetailsUnlockCliff({
  tokenAmountUi,
  tokenPriceUsd,
  tokenSymbol,
  tokenDecimals,
  ...
  const { unlockCliffFluxes, setUnlockCliffFluxes } = useVestingStore((s) => ({
    unlockCliffFluxes: s.unlockCliffFluxes,
    setUnlockCliffFluxes: s.setUnlockCliffFluxes,
  }))
  ...
  const filteredFluxes = searchAddress
    ? unlockCliffFluxes.filter((recipient) =>
  ...
  const remainingBalance =
    typeof tokenAmountUi === 'number'
      ? tokenAmountUi - unlockCliffFluxes.reduce((acc, { amount }) => acc + amount, 0)
      : 0
    ...
    <div className="flex w-full flex-col lg:flex-row items-start gap-x-[1rem] gap-y-[2rem]">
      ...
      <div className="flex w-full lg:max-w-[246px] flex-col gap-y-[1rem] ml-0 lg:ml-[3em]">
        ...
        <RecipientsListWithAccordion
          ...
          setRecipients={setUnlockCliffFluxes}
          tokenSymbol={tokenSymbol}
          emissionType="unlockCliff"
        />
      </div>
    </div>
  )
