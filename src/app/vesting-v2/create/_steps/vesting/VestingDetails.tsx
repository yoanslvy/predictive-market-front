...
export function VestingDetails({
  ...
  // This will run when the component renders and when the selected state changes
  const { fluxes, setLinearFluxes } = useVestingStore((s) => {
    return {
      fluxes: s.fluxes,
      setLinearFluxes: s.setLinearFluxes,
    }
  })
  ...
  const remainingBalance =
    typeof tokenAmountUi === 'number'
      ? tokenAmountUi - fluxes.reduce((acc, { amount }) => acc + amount, 0)
      : 0
  ...
  function onFormSubmit(e: FormEvent<HTMLFormElement>) {
    ...
    if (canAddFlux) {
      ...
      setLinearFluxes((prevFluxes) => {
        const result = [...prevFluxes, newFlux]
        return result
      })
