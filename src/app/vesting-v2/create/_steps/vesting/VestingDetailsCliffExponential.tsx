...
export function VestingDetailsCliffExponential({
  tokenAmountUi,
  tokenPriceUsd,
  tokenSymbol,
  tokenDecimals,
  ...
  const { cliffExponentialFluxes, setCliffExponentialFluxes } = useVestingStore((s) => ({
    cliffExponentialFluxes: s.cliffExponentialFluxes,
    setCliffExponentialFluxes: s.setCliffExponentialFluxes,
  }))
  ...
  const [initialUnlockedAmount, setInitialUnlockedAmount] = useState<number | null>(null)
  ...
  const filteredFluxes = searchAddress
    ? cliffExponentialFluxes.filter((recipient) =>
  ...
  const remainingBalance =
    typeof tokenAmountUi === 'number'
      ? tokenAmountUi - cliffExponentialFluxes.reduce((acc, { amount }) => acc + amount, 0)
      : 0
  ...
  const canAddFlux =
    ...
    dayjs(cliffDate).isAfter(dayjs(startDate)) &&
    initialUnlockedAmount <= amount &&
    cliffAmount <= amount - initialUnlockedAmount &&
    dayjs(startDate).isAfter(dayjs())

  function onFormSubmit(e: FormEvent<HTMLFormElement>) {
    e.preventDefault()
    if (canAddFlux) {
      setCliffExponentialFluxes((prev) => [
        ...prev,
        {
          address: recipient,
          amount,
      ...
      if (!initialUnlockedAmount) {
        toast.error('Please enter an initial unlocked amount.')
        return
      }
      if (initialUnlockedAmount > amount) {
        toast.error('Cliff amount cannot be greater than the total amount.')
        return
      }
      if (cliffAmount > amount - initialUnlockedAmount) {
        toast.error('Cliff amount cannot be greater than the total amount.')
        return
      }
      if (!cliffDate) {
      ...
      if (
        dayjs(cliffDate).isAfter(dayjs(endDate)) ||
        dayjs(cliffDate).isBefore(dayjs(startDate)) ||
        dayjs(cliffDate).isSame(dayjs(startDate))
      ) {
    ...
    <div className="flex w-full flex-col lg:flex-row items-start gap-x-[1rem] gap-y-[2rem]">
        ...
        <>
          {canAddFlux && (
            <Emissions
              ...
              emissions={[
                {
                  ...
                  pv: initialUnlockedAmount,
                },
                {
                  name: new Date(cliffDate),
                  pv: cliffAmount + initialUnlockedAmount,
                },
                {
              ...
              emissionType="CLIFF_EXPONENTIAL"
            />
          )}
        </>
      </Modal>
      <form
        ...
        <AmountInputWithMaxAmount
          onChange={(e) => {
            const n = Number(e)
            setInitialUnlockedAmount(n)
          }}
          amount={initialUnlockedAmount}
          maxAmount={amount ?? undefined}
          tokenSymbol={tokenSymbol}
          tokenPriceUsd={tokenPriceUsd}
          hasInsufficientFunds={hasInsufficientFunds}
          placeholder="Enter initial unlocked amount"
          className="w-full"
          tooltip="The amount that will be released at the start date"
          maxTokenDecimals={tokenDecimals}
        />
        <Divider className="w-full" color="#202228" />
        ...
        <AmountInputWithMaxAmount
          ...
          amount={cliffAmount}
          maxAmount={!amount || !initialUnlockedAmount ? undefined : amount - initialUnlockedAmount}
          tokenSymbol={tokenSymbol}
          tokenPriceUsd={tokenPriceUsd}
          hasInsufficientFunds={hasInsufficientFunds}
          placeholder="Enter cliff amount"
          className="w-full"
          tooltip="The amount that will be released at the cliff date"
          maxTokenDecimals={tokenDecimals}
        />
        <Divider className="w-full" color="#202228" />
        <SingleDatePicker
          ...
          onChange={(value) => {
            if (value) {
              setCliffDate(value ?? null)
            }
          }}
          fromDate={new Date()}
          className="px-[16px] py-[12px] text-[16px] text-white h-[48px]"
          placeholder="Select cliff date"
          tooltip="Select the date when the cliff period ends and the cliff amount is released"
        />
      ...
      <div className="flex w-full lg:max-w-[246px] flex-col gap-y-[1rem] ml-0 lg:ml-[3em]">
        ...
        <RecipientsListWithAccordion
          ...
          setRecipients={setCliffExponentialFluxes}
          tokenSymbol={tokenSymbol}
          emissionType="cliffExponential"
        />
      </div>
    </div>
  )
