...
import { TimelockFlux, useVestingStore } from '../_hooks/useVestingStore'
import { CheckoutTable } from './CheckoutTable'
import { CheckoutVestingSchedule } from './CheckoutVestingSchedule'

export function CheckoutTimelock({
  vestingContract,
  chainId,
  wallet,
  tokenAddress,
  ...
  const { fluxSettings, tokenSymbol, activeFluxes } = useVestingStore((s) => ({
    ...
    activeFluxes: s.activeFluxes as TimelockFlux[],
  }))
              ...
              <CheckoutTable
                ...
                {activeFluxes.map((recipient, idx) => (
                  <tr
                    ...
                    <td className="table-cell h-[4rem] px-[1rem] align-middle text-white">
                      <div className="flex items-center w-full h-full">
                        <span className="min-w-max">{dateFormatter(recipient.unlockDate)}</span>
                      </div>
                    </td>
                  </tr>
                ))}
