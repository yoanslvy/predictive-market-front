...
import { MonthlyUnlocksFlux, useVestingStore } from '../_hooks/useVestingStore'
import { CheckoutTable } from './CheckoutTable'
import { CheckoutVestingSchedule } from './CheckoutVestingSchedule'

export function CheckoutMonthlyUnlocks({
  vestingContract,
  chainId,
  tokenAddress,
  wallet,
  ...
  const { fluxSettings, tokenSymbol, activeFluxes } = useVestingStore((s) => ({
    ...
    activeFluxes: s.activeFluxes as MonthlyUnlocksFlux[],
  }))
              ...
              <CheckoutTable
                ...
                {activeFluxes.map((flux, idx) => (
                  <tr
                    ...
                    <td className="table-cell h-[4rem] px-[1rem] align-middle text-white">
                      <div className="flex items-center w-full h-full">
                        <span className="min-w-max">{shortenEthAddress(flux.address)}</span>
                      </div>
                    </td>
                    <td className="table-cell h-[4rem] px-[1rem]">
                      <div className="flex items-center justify-end w-full h-full">
                        <span className="capitalize min-w-max">
                          {formatAmount(flux.amount)} {tokenSymbol}
                        </span>
                      </div>
                    </td>
                    <td className="table-cell h-[4rem] px-[1rem] align-middle text-white">
                      <div className="flex items-center w-full h-full">
                        <span className="min-w-max">{dateFormatter(flux.startDate)}</span>
                      </div>
                    </td>

                    <td className="table-cell h-[4rem] px-[1rem] align-middle text-white">
                      <div className="flex items-center w-full h-full">
                        <span className="min-w-max">{flux.months}</span>
                      </div>
                    </td>
                    <td className="table-cell h-[4rem] px-[1rem] align-middle text-white">
                      <div className="flex items-center w-full h-full">
                        <span className="flex min-w-max flex-wrap items-center gap-[0.5rem]">
                          {flux.allowTopUps ? (
                            <Check className="size-6 text-[#01EB5A]" />
                          ) : (
                            <X className="text-red-400 size-6" />
                          )}
                        </span>
                      </div>
                    </td>
