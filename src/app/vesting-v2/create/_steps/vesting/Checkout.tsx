...
import { dateFormatter, formatTokenAmountWithDecimals } from '../../../_utils/utils'
...
import { ExponentialFlux, LinearFlux, useVestingStore } from '../_hooks/useVestingStore'
...
export function Checkout({
  ...
  const { fluxSettings, activeFluxes, tokenSymbol } = useVestingStore((s) => ({
    fluxSettings: s.fluxSettings,
    tokenSymbol: s.tokenSymbol,
    activeFluxes: s.activeFluxes,
  }))

  const finalFluxes = activeFluxes as (LinearFlux | ExponentialFlux)[]

  const { isFluxCancellable, isFluxTransferrable } = fluxSettings

  const totalTokenAmount = finalFluxes.reduce((acc, recipient) => {
      ...
      <div className="flex w-full flex-col items-start gap-x-[1rem] gap-y-[1.5em] mt-[2em]">
        ...
        <div className="flex w-full justify-between gap-y-[1rem]">
          <div className="flex w-full flex-col gap-y-[1rem]">
            <CheckoutVestingSchedule recipientCount={finalFluxes.length}>
              <CheckoutTable
                headers={[
                  { jsx: '' },
                  { jsx: 'Recipient' },
                    ...
                    <td className="table-cell h-[4rem] px-[1rem]">
                      <div className="flex items-center justify-end w-full h-full">
                        <span className="capitalize min-w-max">
                          {formatTokenAmountWithDecimals(recipient.amount)} {tokenSymbol}
                        </span>
                      </div>
                    </td>
                    <td className="table-cell h-[4rem] px-[1rem] align-middle text-white">
