...
export default async function TokenTableServer({
  ...
  const vestedLocks = vestedLockDataFetch.vestingV2_vestings.reduce(
    (acc, item) => {
  ...
  const mergedVestingTokens = allTokens
    .reduce(
      (accumulator, current) => {
        const tokens = current.vestingV2_tokens.map((token) => ({
          ...token,
  ...
  return (
    <ErrorBoundary fallback={<TokenTableError />}>
      <Table
        inner
        isLoading={false}
        rowClassName="hover:bg-gray-50 dark:hover:bg-gray-800 cursor-pointer"
        columns={[
          {
            title: 'Token',
            render: ({ data }) => (
              <ClientWrapper
                href={`/vesting-v2/token/chain/${data?.chainId}/address/${data.tokenAddress}`}>
                <TokenAsset
          ...
          },
          {
            ...
            render: ({ data }) => (
              <ClientWrapper
                href={`/vesting-v2/token/chain/${data?.chainId}/address/${data.tokenAddress}`}>
                <div>
                  {[...new Set(data.vestingEmissionTypes)]
                    .map((e) => emissionTypeMapper[e as keyof typeof emissionTypeMapper])
                    .map((e, idx) => {
                      if (idx === 2)
                        return (
